...
class Acto extends Model
{
    ...
    public static function GetPonentes($idActo) {
        return DB::select('SELECT p.* FROM Lista_Ponentes a INNER JOIN Personas p  
            ON a.Id_persona = p.Id_persona AND a.Id_acto = ?',[$idActo]);
    }
    
    public static function IsInscribed($idActo,$idPersona) {
        $resultados = DB::select('SELECT * FROM Inscritos WHERE id_acto = ? AND Id_persona = ?',[$idActo,$idPersona]);
