    ...
    <div class="calendario-container">
        <div class="calendario">
            ...
            <div class='contenedor-eventos'>
                ...
                @endphp
                <form action='/panelusuario/dia?action={{$action}}&dia={{$diaSel}}&mes={{$mes}}&anio={{$anio}}' method='POST' class='evento {{$suscrito}}'>
                    @csrf
            ...
            <div class='contenedor-eventos'>
                ...
                @php
                $suscribed = session('personId') == $ponente->Id_persona;
                $ponente = $suscribed?'evento-ponente':'';
                $idevento = $evento->Id_acto;
                $idInscripcion = $evento->Id_inscripcion;
                $titulo = $evento->Titulo;
                $descripcion = $evento->Descripcion_corta;
                ...
                $asistentes = $evento->Num_asistentes;
                @endphp
                <form method='POST' class='evento-ponente'>
                    <input type='text' name='acto' hidden value='{{$idevento}}'></input>
                    <input type='number' name='inscripcion' hidden value='{{$idInscripcion}}'></input>
                    <!--    <button type='submit'> -->
                    <div class='evento-info'>
